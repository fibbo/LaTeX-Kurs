...
\section*{Vorwort}
This document shall serve as a small summary for working with \LaTeX. It's by far not exhaustive and mostly only touches the surface of the discussed topics. To learn more about any of the packages it's always worth having a look in the manual. The manual can either be found on the web -- just google \texttt{packagename latex} -- or use the provided help function: press windows-key-R and then enter \texttt{texdoc packagename} (Mac users open a terminal and do the same). There is also the \LaTeX\ Wikibook which can be found under \url{https://en.wikibooks.org/wiki/LaTeX}.

If you have specific questions to \LaTeX\ I recommend checking out \url{tex.stackexchange.com}.

\section[Hello World]{Hello World - first example}
\begin{lstlisting}
...
\begin{itemize}
\item \verb+\documentclass[10pt]{article}+ defines the document class (article, report, book, scrartcl, scrreport, scrbook). 10pt indicates default font size
\item \verb+\usepackage[•]{•}+ Used to load packages. In this case, line 2 and 3 allow entering special characters directly.
\item \verb+\begin{document} \end{document}+ is where you put all your content. Everything appearing before or after this command won't show up in your document.
\item \% Used for comments
\end{itemize}

The code preceding \textbf{VOR} \verb+\begin{document}+ is called \textit{preamble}. Most of the setting commands belong there. E.g. \verb+\usepackage[•]{•}+ or commands to set document options.
...
\author{John Smith\\University Here\thanks{bla@gmail.com} \and Michael Smith\\University There\thanks{blub@gmail.com}}
\date{\today}
\begin{document}
...
\begin{itemize}
\item \verb+\usepackage[ngerman]{babel}+ the language package. Used for hyphenation and proper naming of labels and automatic titles
\item \verb+\title{•}+ define the title
\item \verb+\author{•}+ define the authors, combine several with \verb+\and+
\item \verb+\\+ creates a new line (\textbf{NOT} a paragraph)
\item \verb+\date{\today}+ inserts current date. Can be replacec with anything. Leaving it empty suppresses date output
\item \verb+\maketitle+ finally produces the title page at the location of this command
\item \verb+\newpage+ inserts new page
\end{itemize}

Alternatively you can also create your own title page:

\begin{lstlisting}
\begin{titlepage}

% Put here whatever you want in your title page
...
\begin{itemize}
\item \verb+\linespread{x}+ multiplies the standard linespread (=1) by the $x$. \verb+\linespread{2}+ doubles the line spread.
\item \verb+\selectfont+ is needed directly after for the command to work
\end{itemize}
